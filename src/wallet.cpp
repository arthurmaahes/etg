...
bool CWallet::AddToWallet(const CWalletTx& wtxIn)
{
    uint256 hash = wtxIn.GetHash();
    {
        ...
        if (!fInsertedNew)
        {
            ...
            if (wtxIn.nIndex != -1 && (wtxIn.vMerkleEternalGen != wtx.vMerkleEternalGen || wtxIn.nIndex != wtx.nIndex))
            {
                wtx.vMerkleEternalGen = wtxIn.vMerkleEternalGen;
...
bool CWallet::AddToWalletIfInvolvingMe(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fFindBlock)
{
    uint256 hash = tx.GetHash();
    {
        ...
        if (fExisted || IsMine(tx) || IsFromMe(tx))
        {
            ...
            // Get merkle eternal if transaction was found in a block
            if (pblock)
                wtx.SetMerkleEternalGen(pblock);
...
void CWalletTx::AddSupportingTransactions(CTxDB& txdb)
{
    ...
    if (SetMerkleEternalGen() < COPY_DEPTH)
    {
        ...
        // This critsect is OK because txdb is already open
        {
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++)
            {
                ...
                int nDepth = tx.SetMerkleEternalGen();
