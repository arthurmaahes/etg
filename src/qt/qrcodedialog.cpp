...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("ETG")
    updateDisplayUnit();
}
...
{
    QString ret = QString("eternal:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ETG unit input in lnReqAmount, we generate the URI with ETG as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ETG, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
