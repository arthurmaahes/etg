...
/** A transaction with a merkle eternal linking it to the block chain. */
class CMerkleTx : public CTransaction
{
public:
    uint256 hashBlock;
    std::vector<uint256> vMerkleEternalGen;
    int nIndex;
    ...
    (
        ...
        nVersion = this->nVersion;
        READWRITE(hashBlock);
        READWRITE(vMerkleEternalGen);
        READWRITE(nIndex);
    )


    int SetMerkleEternalGen(const CBlock* pblock=NULL);
...
public:
    ...
    std::vector<uint256> GetMerkleEternalGen(int nIndex) const
    {
        ...
        for (int nSize = vtx.size(); nSize > 1; nSize = (nSize + 1) / 2)
        {
            ...
            vMerkleEternalGen.push_back(vMerkleTree[j+i]);
            nIndex >>= 1;
            j += nSize;
        }
        return vMerkleEternalGen;
    }

    static uint256 CheckMerkleEternalGen(uint256 hash, const std::vector<uint256>& vMerkleEternalGen, int nIndex)
    {
        if (nIndex == -1)
            return 0;
        BOOST_FOREACH(const uint256& otherside, vMerkleEternalGen)
        {
            if (nIndex & 1)
 ...
 * to it, but pnext will only point forward to the longest eternal, or will
 ...
 * other node doesn't have the same eternal, it can find a recent common trunk.
...
public:
    ...
    int GetDistanceBack()
    {
        // Retrace how far back it was in the sender's eternal
        int nDistance = 0;
        int nStep = 1;
