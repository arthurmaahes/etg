...
const string strMessageMagic = "EternalGen Signed Message:\n";
...
int CMerkleTx::SetMerkleEternalGen(const CBlock* pblock)
{
    if (fClient)
    {
        if (hashBlock == 0)
    ...
    else
    {
        ...
        {
            vMerkleEternalGen.clear();
            nIndex = -1;
            printf("ERROR: SetMerkleEternalGen() : couldn't find tx in block\n");
            return 0;
        }

        // Fill in merkle eternal
        vMerkleEternalGen = pblock->GetMerkleEternalGen(nIndex);
    }
...
int CMerkleTx::GetDepthInMainChain(CBlockIndex* &pindexRet) const
{
    ...
    // Make sure the merkle eternal connects to this block
    if (!fMerkleVerified)
    {
        if (CBlock::CheckMerkleEternalGen(GetHash(), vMerkleEternalGen, nIndex) != pindex->hashMerkleRoot)
            return 0;
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // EternalGen: 1 days
static const int64 nTargetSpacing = 300; // EternalGen: 2 minute blocks
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal eternal miner
...
bool static Reorganize(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    // Resurrect memory transactions that were in the disconnected eternal
    BOOST_FOREACH(CTransaction& tx, vResurrect)
        tx.AcceptToMemoryPool(txdb, false);

    // Delete redundant memory transactions that are in the connected eternal
...
bool CBlock::SetBestChainInner(CTxDB& txdb, CBlockIndex *pindexNew)
{
    ...
    // Adding to current best eternal
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "EternalGen", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4917e0b5b96deaf46e5b4c0ea2fc9185a597f81fe5dd8b0effba3232f4c4ff4a");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Eternal Generation Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1509499565; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 857562;

        if (fTestNet)
        {
            block.nTime    = 1509499565;
            block.nNonce   = 207577;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x7cc00fd2588127d6c98261cd0b0712fc8ad3bbebdb7528aa2b1d70e35561066c"));
